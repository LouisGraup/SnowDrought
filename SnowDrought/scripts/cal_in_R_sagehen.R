rhessysver = "/work.qigong/RHESSys/bin/rhessys6.0"
...
q.arg = list(list(min=0.2, max=10), list(min=1, max=100), list(min=0.5, max=2.0), list(min=0.5, max=2.0))
...
ecovars = c("scen","date","streamflow")
...
obsflow = read.csv("../streamflow_daily_cal.csv")
#colnames(obsflow)=c("ID","PARAM","bdate","value","err")
obsflow = select(obsflow, Date, Flow_cfs, month, year, day, wy)
#colnames(obsflow) = c("bdate", "flow")
#obsflow$date = mdy(as.Date(obsflow$bdate, format="%m/%d/%Y"))
...
basin.area = 27.2 #km2
stoday = 60*60*24
obsflow$mm = obsflow$Flow_cfs * stoday / (basin.area * 1000 * 1000)*1000

ggplot(obsflow, aes(date, mm))+geom_line()

obs_wy = obsflow %>% group_by(wy) %>% summarize_at(vars(mm), funs(sum))
ggplot(obs_wy, aes(wy, mm))+geom_col()


# set the worldfile, flowtable, header, make sure paths make sense relative to working directory
worldfile = "../worldfiles/sagehen.world.cal"
flowtable = "../flowtables/sagehen.flow"
headrh =  "../worldfiles/sagehen.hdr"
...
cmd2 = sprintf("-pre ../out/test   -s %f %f -sv %f %f -svalt %f %f",
...
cmd3 = sprintf("-st %d 10 1 1 -ed %d 10 1 1 -whdr %s -tchange 0 0 -climrepeat -b -g", startyr, endyr, headrh)
...
a = readin_rhessys_output("../out/test", g=0, wy=0)
...
compa_mth = compa_mwy %>% group_by(month) %>% dplyr::summarize(flow=mean(mm))
